"""Interpolate HaWoR world-space predictions to a target FPS without disps outputs.

Example:
    /bin/python 'scripts/interpolation_no_disps.py' --folder_path example/video_0
"""
...
def _is_slam_done(folder_path, dst_frame_count):
    ...
    slam_files = _collect_slam_50fps_files(folder_path)
    if not slam_files:
        return False
...
def _collect_slam_50fps_files(folder_path):
    slam_dir = os.path.join(folder_path, "SLAM")
    if not os.path.isdir(slam_dir):
        return []
...
def _finalize_existing_slam_50fps(folder_path):
    slam_files = _collect_slam_50fps_files(folder_path)
    if not slam_files:
        return None
    ...
    for slam_file in slam_files:
        try:
            ...
            raise RuntimeError(f"Invalid existing SLAM 50fps npz: {slam_file}") from exc

        frame_count = len(tstamp)
        if frame_count < 1 or traj.shape[0] != frame_count:
            raise ValueError(f"Invalid existing SLAM 50fps frame count: {slam_file}")
...
def main():
    ...
    if not source_exists:
        existing_slam_frame_count = _finalize_existing_slam_50fps(args.folder_path)
        if existing_slam_frame_count is None:
            raise FileNotFoundError(f"Source image folder not found: {source_dir}")

        dst_frame_count = _count_images(target_dir) if target_exists else existing_slam_frame_count
        if target_exists and dst_frame_count != existing_slam_frame_count:
            raise ValueError(
                "Existing SLAM 50fps frame count does not match target image count: "
                f"{existing_slam_frame_count} vs {dst_frame_count}"
            )

        print(f"Source image folder not found: {source_dir}")
        print("Use existing SLAM 50fps npz and skip frame-count-based interpolation.")
        print(f"Image count (timeline): unknown -> {dst_frame_count}")
        print("Check existing outputs:")
        print(f"  SLAM:        {'done' if _is_slam_done(args.folder_path, dst_frame_count) else 'missing/incomplete'}")
        return
